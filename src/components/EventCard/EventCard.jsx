  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow duration-200 flex flex-col">
      ...
      <div className="p-4 flex flex-col flex-1 gap-3">
        ...
        <p className={`text-xs font-medium mt-auto ${seats < 50 ? "text-red-500" : "text-green-600"}`}>
          {seats} seats left
        </p>

        <Link
          to={`/events/${id}`}
          className="mt-1 w-full text-center bg-purple-600 hover:bg-purple-700 text-white text-sm font-semibold py-2.5 rounded-lg transition-colors"
        >
          View Details
        </Link>
      </div>
